...
C150flux<-round(mean(TrapC150$Cavg, na.rm=TRUE), digits=1)/24

days<-360

makeinput<-as.data.frame(matrix(NA,days*24*60*2, 4))
colnames(makeinput)<-c("jday", "hhmm", "s", "POC")
...
makeinput<-makeinput[order(makeinput$jday, makeinput$hhmm, makeinput$s),]


makeinput$POC<-round(rep(C150flux), digits=5)

outfile<-paste("/Users/kasmith/Documents/EnzymeBacteriaModel/BacteriaModel/Experiments/TrapData/BATSin/POM_In_30s_Step.in", sep="")

...
#The code in this file compares the output produced by MRM0D_OptimalEpsilon.R with test files to make sure that the output is the same.  If the files have the same data (to 3 significant digits), then the tests are passed.  A passed test means that optimization analysis code worked!  If the tests fail, the code is not working.
...
TableCompare <- function(filename.new, filename.old) {
	...
	new <- read.table(filename.new, header=TRUE)
	old <- read.table(filename.old, header=TRUE)
...
TableCompare("../Analysis/OptimalEpsilon.txt", "../TestFiles/MRM0D/OptimalEpsilon/OptimalEpsilon.txt")
